...
class MenuArquivo(tk.Menu):
    def __init__(self, master=None, caixa_texto=None, pagina=None):
        ...
        self.add_command(
            label="Abrir...", accelerator="Ctrl+O", command=self.abrir_arquivo
        )
        self.add_command(
            label="Salvar", accelerator="Ctrl+S", command=self.salvar_arquivo
        )
        self.add_command(
            label="Salvar Como...",
            accelerator="Ctrl+Shift+S",
            command=self.salvar_como_arquivo,
        )
    ...
    def salvar_arquivo(self):
        ...
        if titulo_da_pagina_atual == "Sem título - Bloco de Notas":
            ...
            conteudo = self.caixa_texto.get_caixa_texto()
            FileTxt.salvar_arquivo_texto(
                conteudo, self.pagina, self.endereco_do_arquivo
            )
